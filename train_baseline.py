...
train_baseline.py
-----------------
Vanilla DDP training — no straggler detection, no AMP toggling, pure FP32.
Serves as the control baseline to compare against train_with_straggler.py.

Usage
-----
    torchrun --standalone --nproc_per_node=4 train_baseline.py 50 5 --batch_size 32
"""
...
def load_train_objs():
    ...
    model     = torch.nn.Linear(20, 2)   # output dim must match num_class
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Baseline DDP training — no straggler mitigation")
